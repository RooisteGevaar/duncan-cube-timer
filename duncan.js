    ...
    .then((data) => {
      ...
      entries.forEach((entry) => {
        ...
        if (entry.content.$t.charAt(0) !== "#") {
          ...
          apiSheetNumbers.push(pageCount+1)
          console.log(entry.content.$t)
          console.log("page pushed: " + entry.content.$t + ", " + (pageCount+1).toString())
...
function fetchSheetData() {
  ...
  apiSheetNames.forEach((name, i) => {
    let sheetNumber = `${apiSheetNumbers[i]}`;
